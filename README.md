# Gen3MAF
I created this utility to aid in the tuning of the MAF curve on a GEN 3 LS, particularly the LQ4 in my 2003 GMC Yukon Denali.

I am a novice to the tuning world. The only reason I bought the tuner hardware was to enable the e-fans in my vehicle when converted from mechanical.

Since I paid $400 for the product, I decided to see what else I could do with it, so I started playing with tuning the MAF and VE tables. In the process I felt like I would like more systematic way of managing the correction data and how it is applied to create a new MAF curve.

I am guessing the audience for this app is mostly people like me. I'm thinking a professional tuner will probably not have so much use for this.

The app logically fits in between the workflow between the tuner editor and the tuner scanning app. You input the current MAF curve of your vehicle and the feedback back data from the scanner histogram data of the STFT or AFR err.

In the app you can look at the resulting new MAF curve graphically and apply adjustment as you see fit. The app can optionally linearly interpolate between buckets missing correction data. You can adjust what percentage of the correction is applied to the existing curve. You can set a threshold that that must be exceeded to have the correction applied. You can also restrict the range of frequencies that the correction is applied to.
...
The app records each cycle of of generating a new curve from the scanner correction data. You can review each one later.

The application also supports two modes of processing feedback data. A single bucket mode where it applies a single bucket of histogram correction data to each frequency in the curve. This is like how you would do it now, except the bucket is aligned over the frequency. It also supports generating three histogram buckets of correction data for each frequency in the curve. The goal of using three buckets is reduce noise from the correction data.

The app is pretty simple and utilitarian. Hopefully, the workflow it pretty obvious.
...
After that you choose how many buckets you want. Single bucket most closely resembles what you normally do with the tuning editor and scanner. The one difference is that the bucket it centered on the frequency instead of after it. 

With triple buckets there is one bucket centered on the target frequency and one 1/3 of the distance to the adjoining frequency targets. When processing the correction data the app tries to pick an average value for the final correction value. This was really my whole motivation for writing the app.
...
The top TextBox contains bucket data that you would paste into the scanner histogram configuration.
...
At this point you can pause your work and come back to it later when you have collected the correction data from a drive. Let s assume you already have it.
...
The TextBox will likely be empty because there are a lot of tabs with no data to be displayed, but a scrol bar will appear and you can scroll to verify that there is something there. Now press the button to Apply the adjustments. The GridView below will fill out.
...
3. You can have the program interpolated between buckets with no correction data. If it is for the initial correction from stock, this is probably fine. For more fine adjustments you may want to turn it off.
4. You can constrain the range of frequencies that the correction data will be applied to. For example, with you are tunning WOT P.E stuff and you have a bunch of random correction data at low airflow rates, you can exclude those.
...
The blue line is the original MAF curve, The yellow line is the curve with the correction data applied. The diamond markers are the actual frequency points from the MAF curve. The small markers are the extra tuning points added for the triple buckets.
